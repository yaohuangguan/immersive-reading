    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute top-6 right-6 z-20">
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2 bg-slate-800/50 backdrop-blur border border-slate-700 rounded-full text-slate-300 hover:text-white hover:bg-slate-800 transition-all"
        >
          <Globe size={16} />
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 opacity-20 pointer-events-none">
        <motion.div 
          ...
          className="absolute -top-40 -right-40 w-96 h-96 bg-purple-600 rounded-full blur-[100px]" 
        />
        <motion.div 
           animate={{ scale: [1, 1.5, 1], rotate: [0, -45, 0] }} 
           transition={{ duration: 15, repeat: Infinity }}
          className="absolute top-40 -left-20 w-72 h-72 bg-blue-600 rounded-full blur-[80px]" 
        />
      </div>
      ...
      <div className="max-w-3xl w-full text-center space-y-8 z-10">
        ...
        >
          ...
          <h1 className="text-5xl md:text-7xl font-bold font-serif mb-4 bg-clip-text text-transparent bg-gradient-to-r from-white via-indigo-200 to-indigo-400">
            ImmerseRead
          </h1>
        ...
        >
            {isLoading ? (
                ...
                <div className="grid md:grid-cols-2 gap-8 items-center">
                    {/* Drag Drop Zone */}
                    <div 
                        className={`border-2 border-dashed rounded-2xl p-8 flex flex-col items-center justify-center transition-all cursor-pointer h-full min-h-[250px]
                        ${dragActive ? "border-indigo-400 bg-indigo-500/10" : "border-slate-600 hover:border-indigo-400/50 hover:bg-slate-700/30"}`}
                    ...
                    >
                        ...
                        <p className="text-sm text-slate-500">{t.hero.formats}</p>
                        <input 
                            id="file-upload" 
                            type="file" 
                            className="hidden" 
                    ...
                    <div className="flex flex-col h-full justify-center space-y-6">
                        ...
                        <form onSubmit={handleManualSubmit} className="space-y-4">
                            ...
                            <button 
                                type="submit"
                                disabled={!title.trim()}
                                className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-semibold py-4 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20"
                            >
                                <Sparkles size={20} />
                                {t.hero.button}
                            </button>
